...
usage() {
  cat <<'USAGE'
Usage:
  litefury-spi-verify.sh [--dry-run] IMAGE_PATH ADDRESS_HEX SIZE_BYTES

GoldenGate backend adapter for verifying LiteFury SPI flash contents through
the RHS `spi-loader` command. This uses spi-loader compare/check mode rather
than dumping a raw readback file.
USAGE
}
...
printf 'litefury_spi_verify_start_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
printf 'image=%s\n' "${IMAGE}"
printf 'address=%s\n' "${ADDRESS}"
printf 'size_bytes=%s\n' "${SIZE_BYTES}"
printf 'spi_loader=%s\n' "${loader}"
...
lf_run_locked "${DRY_RUN}" "${lf_spi_timeout_seconds}" "${lf_spi_lock_wait_seconds}" \
  ...
  -t "${lf_spi_target}" -f "${IMAGE}" -l "${SIZE_BYTES}" -c -v

printf 'litefury_spi_verify_done=1\n'
