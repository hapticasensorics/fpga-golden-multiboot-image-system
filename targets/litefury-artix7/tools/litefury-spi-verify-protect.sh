...
printf 'litefury_spi_verify_protect_start_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  output="$(lf_spi_protect_run "${DRY_RUN}" "${tool}" --status "${range_args[@]}")"
  printf '%s\n' "${output}"
  printf '%s\n' "${output}" | grep -q '^ppb_range_fully_protected=1$' ||
    lf_die "golden range is not fully PPB protected"
fi
